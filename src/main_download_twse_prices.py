...
logging.basicConfig(level=logging.INFO, filename='log/main_download_twse_prices.log', filemode='w', format=FORMAT)
...
def main():
    ...
    with open('data/twse_tickers.json', 'r', encoding='utf8') as f:
        tickers = json.loads(f.read())

    logging.info('total tickers, %s' % len(tickers))

    working_dir = os.getcwd()
    dst_dir = '%s/data/twse/%s' % (working_dir, str(dt.today().date()))
    try:
    ...
    run_async(ticker_lists, dst_dir, 'max')
